# Fallout Pip-Boy Theme for Innioasis Y1

This repository contains a series of themes for the Innioasis Y1 music player, styled after the **Pip-Boy** screen from the Fallout game series.

## 🎨 Theme Description

This **yellow variant** is inspired by the visual style of Fallout: New Vegas.  
The interface uses original game icons along with hand-drawn elements adapted for Y1 firmware limitations.  
The theme is maximally detailed based on all available information from official resources and the r/innioasis community.

Available color schemes: **pink**, **classic green**, and **mint** versions.  
The default calendar icon (with number 7) in "Date & Time" settings looks organic only in the mint version.

## 🖼️ Screenshots

![Fallout Pip-Boy 1Y Theme Demo](screenshot.gif)

## 🎨 Custom Wallpapers

**Personalize your Pip-Boy!** You can easily change wallpapers:

### Current settings in `config.json`:
"desktopWallpaper": "desk-1.png",
...
| **1** *(default)* | `desk-1.png` | `globalWallpaper-1.png` | Light screen color effect |
| **2** | `desk-2.png` | `globalWallpaper-2.png` | Strong screen color effect |

## 🎇Screen Defect Mask (Optional)

The theme also supports optional **screen defect styling** using mask images.  
This can simulate scratches, dust, or other display imperfections for a more atmospheric look.

By default, this feature is **disabled**. In `config.json` the mask parameters are empty:
...
"settingMask": ""
```
Сhange the empty value to enable the mask on the main screen and the settings screen:
...
**Fallout, Pip-Boy** and all related elements belong to their rightful owners.  
This is **non-commercial fan art** created out of love for the Fallout franchise.

🎮 Huge thanks to Bethesda and Fallout creators for the inspiration!

## 📋 [RUS] Quick Overview

**Fallout Pip-Boy тема** для плеера Innioasis Y1 (жёлтая, розовая, зелёная, мятная). 🎮

✅ Максимальная детализация интерфейса с оригинальными иконками  
❌ Ограничения: статус-бар (вибрация/щелчки), календарь в настройках  
🙏 Ищу помощь по недостающим параметрам  
© Фанатское творчество, все права Fallout — Bethesda

    ...
    <script>
        ...
        async function checkFeatureSupport() {
            try {
                ...
                // Only check the 4 specific upcoming/region-specific features in settingConfig
                const upcomingFeaturesToCheck = [
                    { key: 'ebook', displayName: 'E-book Reader', section: 'settingConfig', configKey: 'ebook' },
                    { key: 'calculator', displayName: 'Calculator', section: 'settingConfig', configKey: 'calculator' },
                    { key: 'calendar', displayName: 'Calendar', section: 'settingConfig', configKey: 'calendar' },
                    { key: 'launcher', displayName: 'Rockbox / Alternative Launcher Icon', section: 'settingConfig', configKey: 'launcher' }
                ];
                ...
                if (unsupportedFeatures.length > 0) {
                    ...
                    if (section && list) {
                        list.innerHTML = unsupportedFeatures.map(feature => {
                            // Create GitHub link to config.json
                            const githubUrl = `https://github.com/y1-community/InnioasisY1Themes/blob/main/${encodeURIComponent(folderName)}/config.json`;
                            return `<li style="margin-bottom: 5px;"><a href="${githubUrl}" target="_blank" rel="noopener noreferrer" style="color: #667eea; text-decoration: none; border-bottom: 1px solid #667eea;">${feature.displayName}</a> - "${feature.configKey}" in "settingConfig"</li>`;
                        }).join('');

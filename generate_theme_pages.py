    ...
    <script>
        ...
        async function loadThemeImages() {
            ...
            try {
                ...
                // Only show images that are defined in config.json
                // First, get all image filenames from config.json (with all variations)
                const configImageFiles = new Set();
                imageUsageMap.forEach((keys, filename) => {
                    configImageFiles.add(filename);
                });
                
                // Test which images actually exist AND are in config.json
                // This filters out non-existent files AND caches them for download/install
                const loadedImages = [];
                const loadPromises = imageFiles.map((image) => {
                    return new Promise(async (resolve) => {
                        // Check if this image is referenced in config.json
                        const imageName = image.name;
                        let isInConfig = configImageFiles.has(imageName);
                        
                        // Try variations if not found
                        if (!isInConfig) {
                            const nameWithoutPrefix = imageName.replace(/^@[^_]+_/, '');
                            isInConfig = configImageFiles.has(nameWithoutPrefix);
                        }
                        if (!isInConfig) {
                            const baseName = imageName.replace(/\.[^.]+$/, '');
                            isInConfig = configImageFiles.has(baseName);
                        }
                        if (!isInConfig) {
                            const baseNameWithoutPrefix = imageName.replace(/^@[^_]+_/, '').replace(/\.[^.]+$/, '');
                            isInConfig = configImageFiles.has(baseNameWithoutPrefix);
                        }
                        
                        // Only process images that are in config.json
                        if (!isInConfig) {
                            resolve();
                            return;
                        }
                ...
                loadedImages.sort((a, b) => {
                    // Get config keys for sorting
                    const aKeys = imageUsageMap.get(a.name) || imageUsageMap.get(a.name.replace(/^@[^_]+_/, '')) || imageUsageMap.get(a.name.replace(/\.[^.]+$/, '')) || [];
                    const bKeys = imageUsageMap.get(b.name) || imageUsageMap.get(b.name.replace(/^@[^_]+_/, '')) || imageUsageMap.get(b.name.replace(/\.[^.]+$/, '')) || [];
                    const aKey = aKeys[0] || a.name;
                    const bKey = bKeys[0] || b.name;
                ...
                // Create carousel items for successfully loaded images that are in config.json
                ...
                loadedImages.forEach((image) => {
                    ...
                    // Get config.json usage for this image
                    // Try multiple lookup strategies (already handled in the map creation)
                    const imageName = image.name;
                    let usageKeys = imageUsageMap.get(imageName);
                    
                    // If not found, try without @ prefix pattern
                    if (!usageKeys || usageKeys.length === 0) {
                        const nameWithoutPrefix = imageName.replace(/^@[^_]+_/, '');
                        usageKeys = imageUsageMap.get(nameWithoutPrefix);
                    }
                    
                    // If still not found, try just the base name without extension
                    if (!usageKeys || usageKeys.length === 0) {
                        const baseName = imageName.replace(/\.[^.]+$/, '');
                        usageKeys = imageUsageMap.get(baseName);
                    }
                    
                    // Also try base name without prefix
                    if (!usageKeys || usageKeys.length === 0) {
                        const baseNameWithoutPrefix = imageName.replace(/^@[^_]+_/, '').replace(/\.[^.]+$/, '');
                        usageKeys = imageUsageMap.get(baseNameWithoutPrefix);
                    }
                    
                    // Display config key name(s) - should always be found since we filtered above
                    if (usageKeys && usageKeys.length > 0) {
                        // Show the config key name(s) - join multiple with comma
                        label.textContent = usageKeys.join(', ');
                        label.title = image.name; // Show filename on hover
                    }

...
def get_font(folder):
    # Check for font file with various extensions
    extensions = ['*.ttf', '*.otf', '*.woff', '*.woff2']
    for ext in extensions:
        fonts = glob.glob(os.path.join(folder, ext))
        if fonts:
            return os.path.basename(fonts[0])
            
    # Also check for "Font.*" specifically (case insensitive)
    for file in os.listdir(folder):
        if file.lower().startswith('font.') and file.lower().endswith(('.ttf', '.otf', '.woff', '.woff2')):
            return file
...
<head>
    ...
    <style>
        ...
        .install-btn {
            background: linear-gradient(135deg, #00b09b 0%, #96c93d 100%); /* Green gradient for install */
...
<body>
    ...
    <script>
        console.log('✅ Main script tag loaded and executing');
        ...
        // Load theme background dynamically
        async function loadThemeBackground() {
            try {
                // Try to load config.json to find background
                const configResponse = await fetch('./config.json');
                if (!configResponse.ok) return;
                
                const config = await configResponse.json();
                
                // Priority list for background images
                const bgCandidates = [
                    config.desktopWallpaper,
                ...
                // Try to load candidates one by one
                for (const filename of bgCandidates) {
                    // Clean filename (remove path if present, though config usually has just filename)
                    const cleanName = filename.split('/').pop();
                    const url = `./${encodeURIComponent(cleanName)}`;
                    
                    try {
                        const response = await fetch(url, { method: 'HEAD' });
                        if (response.ok) {
                            // Found a valid background!
                            const bgElement = document.querySelector('.bg-cycle');
                            if (bgElement) {
                                bgElement.style.backgroundImage = `url('${url}')`;
                                bgElement.style.opacity = '1';
                                console.log(`Loaded background: ${cleanName}`);
                                return;
                            }
                        }
                    } catch (e) {
                ...
                console.warn('Error loading theme background:', e);
            }
        }
        ...
        async function loadThemeImages() {
            console.log('🖼️ loadThemeImages() called');
            const folderName = '{folder}';
            ...
            try {
                ...
                // Strategy 2: Use files found in config.json!
                // This is critical if GitHub API fails and files aren't in the common list
                if (imageUsageMap.size > 0) {
                    const configFiles = new Set();
                    // Extract original filenames from the map keys (we need to be careful as keys include variations)
                    // But we can just try to load all unique keys that look like filenames
                    imageUsageMap.forEach((_, key) => {
                        if (/\.(png|jpg|jpeg|gif|svg)$/i.test(key)) {
                            configFiles.add(key);
                        }
                    });
                    
                    configFiles.forEach(filename => {
                        // Check if already in imageFiles
                ...
                // Always show the carousel section if we have config.json or images
                // Don't hide it just because imageFiles might be empty (though it shouldn't be now)
                const carouselSection = document.getElementById('image-carousel-section');
                if (carouselSection) {
                    carouselSection.style.display = 'block';
                }
                
                if (imageFiles.length === 0 && !config) {
                ...
                loadedImageData.forEach((imageData) => {
                    ...
                    // Clean up the label: remove section prefix if present (e.g., "itemConfig.itemSelectedBackground" -> "itemSelectedBackground")
                    const cleanLabel = imageData.configKey.split('.').pop();
                    label.textContent = cleanLabel;
                    label.title = `${imageData.configKey} (${imageData.filename})`; // Show full info on hover
        ...
        document.addEventListener('DOMContentLoaded', () => {
            console.log('🎯 DOMContentLoaded fired, calling loadThemeImages...');
            loadThemeImages();
            loadThemeBackground();
        });
    </script>

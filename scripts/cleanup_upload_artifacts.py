...
Upload flow writes `<name>.zip` plus `<name>.zip.meta.json`.
This script removes:
- orphan sidecars (meta without zip)
- stale zip/meta pairs older than a retention window (only when paired with sidecar)

Retention is controlled by `UPLOAD_ARTIFACT_MAX_AGE_HOURS` (default: 48).
Set to `0` to remove all matching upload artifacts immediately.
"""

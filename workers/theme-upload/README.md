...
Each upload opens a PR that adds a root **`gallery-upload-*.zip`** (often a batch archive whose members are only inner `*.zip` theme packages) plus a sibling **`*.zip.meta.json`** (`shareFolderHints` lists the expected theme folder name(s) for post-merge share links). Actions [`scripts/process_theme_zips.py`](../../scripts/process_theme_zips.py) unpacks each inner zip into theme folders, then removes the outer archive. [`scripts/validate_theme_pr.py`](../../scripts/validate_theme_pr.py) allows the sidecar only when the matching zip is in the same PR. For **auto-merge**, leave [`GITHUB_ZIP_UPLOAD_DIR`](../../functions/_lib/theme-upload-handler.js) empty so both files sit at repo root (nested zip paths are rejected by the validator today).

On **main**, root ZIP ingestion, `themes.json` sync, and cleanup of processed zips/meta are handled by [`.github/workflows/theme-ingest-and-sync.yml`](../../.github/workflows/theme-ingest-and-sync.yml) (it no longer bulk-regenerates every theme `index.html` or `sitemap.xml` to avoid merge races). [`.github/workflows/generate-sitemap.yml`](../../.github/workflows/generate-sitemap.yml) refreshes the sitemap on a schedule. Manual [`.github/workflows/sync-theme-metadata.yml`](../../.github/workflows/sync-theme-metadata.yml) remains available for metadata-only fixes.
...
That happens when root `wrangler.toml` has **`pages_build_output_dir`** (Wrangler treats it as Pages and ignores `main`) or when `main` is missing/wrong. **Fix:** pull latest `main` — root `wrangler.toml` is Worker-only; Pages settings moved to `wrangler.pages.toml`. Retry the Worker deployment.
